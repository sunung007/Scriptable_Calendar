...
//  const lineNum = Math.ceil((firstDay+lastDate)/7.0)
  const matrix = [new Array(31), new Array(31)] // 당일, 기간
  const weekStart = [1]
  ...
  for(let i = 1 ; i <= lastDate ; i++) {
    tempCalendar[i] = new Array()
  }
  
  
  for(let i = 8-firstDay ; i < lastDate ; i += 7) {
    if(i > 1) weekStart.push(i)
  }

  const getWeekEnds = (day) => {
    let first, last
  ...
  for(let i in calendarJSON) {
    ...
    if(startDate == endDate) {
      temp.isPeriod = calendarJSON[i].isAllDay
      temp.title = calendarJSON[i].title
      
      temp.startDate = calendarJSON[i].startDate
      temp.endDate = calendarJSON[i].endDate
      
      temp.start = startDate
      temp.end = endDate

      tempCalendar[startDate].push(temp)
    }
    // 기간
    else {
      temp.isPeriod = true
      ...
      else if(startMonth > thisMonth) { // from last year.
        startDate = 1
      }
      ...
      addToMatrix(startDate, endDate, temp)
    }
  }

  function addToMatrix(start, end, json) {
    for(let i = start ; i <= end ; i++) {
      tempCalendar[i].push(json)
    }
  }
