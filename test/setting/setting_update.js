...
const
  background_list = ['단색으로 설정', '이미지에서 선택', '북마크에서 선택', '새로운 투명위젯'],
  color_list = ['검정색','하얀색','노란색','초록색','파란색','시스템'],
  size_list = ['작은 크기', '중간 크기', '큰 크기'],
  show_list = ['보이기', '보이지 않기'],
  large_list = ['캘린더만', '리마인더만', '캘린더 + 리마인더'],
  align_list = ['왼쪽 정렬', '오른쪽 정렬'],
  calendar_list = ['오늘 일정만', '이번 주 일정 보기', '이번 달 일정 보기', '7일간 일정 보기', '30일간 일정 보기', '기타'],
  language_list = ['한국어', '영어'];

let
  calendar_source_list = [],
  reminder_source_list = [],
  calendar_source_status = [],
  reminder_source_status = [];

const
  all_calendar_source = await Calendar.forEvents(),
  all_reminder_source = await Calendar.forReminders();

let
  saved_calendar_source,
  saved_reminder_source;

if(settingJSON.calendarSource != null) {
  saved_calendar_source = settingJSON.calendarSource.split(',');
}
if(settingJSON.reminderSource != null) {
  saved_reminder_source = settingJSON.reminderSource.split(',');
}

for(let i in all_calendar_source) {
  calendar_source_list.push(all_calendar_source[i].title);
  for(let saved of saved_calendar_source) {
    if(saved === calendar_source_list[i]) {
...
for(let i in all_reminder_source) {
  reminder_source_list.push(all_reminder_source[i].title);
  for(let j in saved_reminder_source) {
    if(saved_reminder_source[j] === reminder_source_list[i]) {
      reminder_source_status.push(true);
      break;
    }
  }
  if(reminder_source_status[i] !== true) {
    reminder_source_status[i] = false;
  }
}
...
let
  background_change = settingJSON.backgroundType == null ? 0 : settingJSON.backgroundType,
  back_color_change = settingJSON.backgroundColorNumber == null ? 0 : Number(settingJSON.backgroundColorNumber),

  background_image,
  background_bookmark,

  content_change = settingJSON.contentColor == null ? 0 : Number(settingJSON.contentColor),

  size_change = settingJSON.widgetSize == null ? 0 : Number(settingJSON.widgetSize) - 1,
  large_setting = settingJSON.largeWidgetSetting == null ? ['true','true','true'] : settingJSON.largeWidgetSetting.split(','),
  monthly_change = large_setting[2] === 'true' ? 0 : 1,
  large_change,

  calendar_time_change = settingJSON.showCalendarTime === 'true' ? 0 : 1,
  align_change = settingJSON.isCalendarRight === 'true' ? 1 : 0,
  calendar_change = settingJSON.calendarPeriod,
  calendar_period = 0,
  language_change = settingJSON.locale === 'en' ? 1 : 0;

if(background_change === 'color') {
  background_change = 0;
}
...
else if(background_change === 'invisible') {
  background_change = 3;
}

if(large_setting[0] === 'true' &&
  large_setting[1] === 'true')       large_change = 2;
else if(large_setting[0] === 'true') large_change = 0;
else if(large_setting[1] === 'true') large_change = 1;

if(calendar_change === undefined ||
  calendar_change === 'today')            calendar_change = 0;
...
const background_row = () => {
  ...
  background_left.onTap = () => {
    background_change = background_change === 0 ? 3 : background_change-1;
  ...
  color_left.onTap = () => {
    back_color_change = back_color_change === 0 ? 5 : back_color_change-1
    rows[4] = background_row()
    refreshAllRows()
  }
  ...
  if(size_change === 1) {
    ...
    if(large_change === 0 || large_change === 2) {
      for(let i in calendar_source_list) {
       ...
       // Remidner source
    row = new UITableRow()
    cell = UITableCell.text('','미리알림 종류 선택')
    row.addCell(cell)
    if(large_change >= 1) arr.push(row)

    if(large_change >= 1) {
      for(let i in reminder_source_list) {
        row = new UITableRow()
        cell = UITableCell.button(reminder_source_list[i] + '  |  ' + (reminder_source_status[i] ? '보기' : '보지않기'))
        cell.centerAligned()
        row.addCell(cell)
        ...
        cell.onTap = () => {
          reminder_source_status[i] = !reminder_source_status[i]
          rows[10] = size_row()
          refreshAllRows()
        }
      }
...
async function saveSetting() {
  ...
  let calendarSources = [], reminderSources = []
  if(size_change === 1) {
    newJSON.largeWidgetSetting = [large_change!==1, large_change!==0]
    ...
    if(large_change !== 1) {
      ...
      newJSON.showCalendarTime = calendar_time_change===0 ? 'true':'false'

      for(let list of calendar_source_list) {
        if(list) {
          calendarSources.push(list)
        }
      }
    }
    ...
    if(large_change !== 0) {
      for(let i in reminder_source_list) {
        if(reminder_source_status[i]) {
          reminderSources.push(reminder_source_list[i])
        }
      }
    }
    ...
    newJSON.reminderSource = reminderSources.toString()
  }
  else {
    for(let i in calendar_source_list) {
  ...
  noti.title = '[Gofo] 달력 위젯 설정 완료'
  noti.body = '달력 위젯 설정이 완료되었습니다.'
  noti.schedule()
